...
export async function _FileConverter(ext, DIR, Fn) {
    try {
        ...
        const list = await readDirectory(DEST_DIR, ext);
        list.push(manual);
        const fileselect = await checkbox({
            message: `Select found ${ext} files`,
            choices: list.map((file) => ({ name: file, value: file })),
        });
...
export async function _ConcatFiles(DIR, Fn) {
    try {
        let ext = '';
        const DEST_DIR = process.cwd();
        const DEST = DIR ? `${DEST_DIR}\\${DIR}` : DEST_DIR;
