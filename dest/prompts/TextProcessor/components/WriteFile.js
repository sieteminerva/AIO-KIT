 ...
 */
export function SetDefaultFilename(filepath, engine, qty) {
    let defaultName = '';
    let ext = extname(filepath);
    const selectedFilename = basename(filepath, ext);
 ...
 */
export async function CreateWriteFilePrompt(filepath, engine, multiple = false) {
    try {
        ...
        if (confirmWrite) {
            const inputName = await input({
                ...
                default: multiple ? SetDefaultFilename(filepath, engine, multiple) : SetDefaultFilename(filepath, engine),
            });
