 ...
 * ```
 */
export async function processFile(processor, filepath, engineName, options) {
    let result = '';
    switch (engineName) {
        case 'findMatch':
            result = await processor.findMatch(filepath, options);
            break;
        case 'split':
            result = await processor.split(filepath, options.chunkSize);
            break;
        case 'replace':
        case 'translate':
            result = await processor[engineName](filepath, options);
            break;
        case 'purify':
        case 'analyze':
        case 'summarized':
        case 'translate':
            result = await processor[engineName](filepath);
            break;
        default:
