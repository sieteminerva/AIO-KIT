 ...
 * ```
 */
export async function SelectImageFilesPrompt(folder: Dirent[]): Promise<string[] | undefined> {
  try {
    if (folder.length === 0) {
    ...
    const files = await checkbox({
      ...
      choices,
      validate(choices) {
        return imageFilesValidation(choices as any);
      },
    });
    return files;
  } catch (error) {
 ...
 */
function imageFilesValidation(files: any[]) {
  // must select a file
  if (files.length === 0) {
