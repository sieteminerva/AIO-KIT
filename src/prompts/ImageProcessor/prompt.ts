 ...
 */
export async function SelectImageTaskDialog(filename: string, DIR: string) {
  ...
  try {
    ...
    if (menu === "optimize") await ImageOptimizerPrompt(DIR, filename)
    else if (menu === 'convert') await ImageConverterPrompt(DIR, filename);
    else if (menu === 'back') await SelectUtilityPrompt(DIR, filename)
    else if (menu === 'quit') process.exit(0);
