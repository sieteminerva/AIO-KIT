...
import { _ConcatFiles, _FileConverter } from "./Utility.prompt.js";
 ...
 */
export async function SelectUtilityPrompt(DIR: string, filename?: string) {
  ...
  try {
    ...
    if (task) {
      switch (task) {
        case 'image-convert':
          await ImageConverterPrompt(DIR, filename as string);
          break;
        case 'image-optimizer':
          await ImageOptimizerPrompt(DIR, filename);
          break;
        case 'file-convert':
          await SelectFileConverterPrompt(DIR, filename as string);
          break;
        case 'compile':
          await CompileDeclarationsFilesPrompt(DIR, filename as string);
          break;
        case 'back':
          await MainDialogPrompt(filename as string, DIR);
          break;
        case 'quit':
          process.exit(0);
      }
