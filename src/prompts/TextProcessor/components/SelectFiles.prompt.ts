...
export async function SelectTextFilesPrompt(folder: Dirent[], engine: AioTextProcessorMethods) {
  /**
  ...
  try {
    ...
    const files = await checkbox({
      ...
      choices,
      validate(choices) {
        return textFilesValidation(choices as any, engine);
      },
    });
    return files;
  } catch (error) {
 ...
 * ```
 */
function textFilesValidation(files: any[], engine: string) {
  // must select a file
  if (files.length === 0) {
