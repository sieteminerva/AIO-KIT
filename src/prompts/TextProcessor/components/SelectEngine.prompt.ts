 ...
 * }
 */
export async function CreateSelectEnginePrompt(): Promise<AioTextProcessorMethods | string | undefined> {

  const online = await isOnline();
  const msg = `'Translate & Summarize features are '${redBright('DISABLED')},\nbecause your not connected to the Internet!'`
  if (!online) console.log(msg);

  try {
    const selected: AioTextProcessorMethods | string = await select({
