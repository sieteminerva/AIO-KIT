 ...
 */
export async function DataGeneratorPrompt(DIR: string, schema?: any) {
  ...
  try {
    ...
    const choices = [
      ...DATA_TYPES.map((menu: any) => ({ name: menu.name, value: menu.value })),
    ...
    if (dataType === 'custom') {
      ...
      const filename = await input({ message: 'Output Filename?', default: dataType as string });
