...
import { readdir, open, writeFile, rm, FileHandle } from "node:fs/promises";
...
export class CompileDeclarationsFiles {
  ...
  private _dtsExt: string;
  private _jsExt: string;
   ...
   * ```
   */
  async checkOutputFiles(dtsFileName: string, jsFileName: string): Promise<{ dts: string; js: string }> {
  ...
  async concatenateFiles(files: Dirent[]): Promise<string[]> {
    const docs: string[] = [];
   ...
   * ```
   */
  private generateExports(docs: string[], outputDtsPath: string): string[] {
  ...
  filterFnClass(content: string[]): string[] {
    ...
    const pattern: RegExp = /(?:function|class)\s+([a-zA-Z_$][0-9a-zA-Z_$]*)\s*\(|class\s+([a-zA-Z_$][0-9a-zA-Z_$]*)\s*(?:extends|implements|\{)/;
    ...
    for (const line of content) {
      if (line) {
        const fnClassMatch = line.match(pattern as RegExp);
        if (fnClassMatch) {
   ...
   * ```
   */
  format(task: string, content: string[], filePath: string) {
   ...
   * @example
   */
  async search(directory: string, ext: string): Promise<Dirent[]> {
   ...
   * @example
   */
  async write(outputFilePath: string, content: string[]): Promise<void> {
