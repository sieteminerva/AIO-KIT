...
export class AioDataGenerator {
   ...
   * ```
   */
  public setSchema(type: 'custom' | string, schema?: any) {
    let usedSchema;
    if (type !== 'custom') {
  ...
  public getSchema(type: string) {
    ...
    const schemas: any = merge(uniSchema, aisSchema)
   ...
   * ```
   */
  public generateData(type: string, amount: number, createId: boolean, createIndex: boolean, schema?: any) {
    const data = [];
    if (schema === 'summary') {
      return this.setSchema('summary');
    } else {
      for (let i = 0; i < amount; i++) {
        let item: any = {}; // Create a new object for each item
        if (type !== 'custom') {
        ...
        if (createId) item.key = generateId();
        if (createIndex) item.index = i + 1;
